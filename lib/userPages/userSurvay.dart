...
class UserSurvayPage extends StatefulWidget {
  final workerId;
  const UserSurvayPage({this.workerId, Key? key}) : super(key: key);

  @override
  State<UserSurvayPage> createState() => _UserSurvayPageState();
}
...
class _UserSurvayPageState extends State<UserSurvayPage>
...
dynamic var1=0.0,var2=0.0,var3=0.0,var4=0,var5=0.0;
    ...
    print(x);
    await FirebaseFirestore.instance.collection('worker').doc("${widget.workerId}").get().then((value) {
      y=value.data()!['rate'];
      print(y);
    });
    var z=x+y;
    print(z);
   await FirebaseFirestore.instance.collection('worker').doc("${widget.workerId}").update({
      'rate':z,
    });
